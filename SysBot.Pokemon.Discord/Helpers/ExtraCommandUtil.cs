...
namespace SysBot.Pokemon.Discord
{
    public class ExtraCommandUtil<T> where T : PKM, new()
    {
        ...
        public static async Task HandleReactionAsync(Cacheable<IUserMessage, ulong> cachedMsg, Cacheable<IMessageChannel,ulong> cachedchannel, SocketReaction reaction)
        {
            bool hasEmbed = cachedMsg.HasValue && cachedMsg.Value.Embeds.Count > 0;
            if (!reaction.User.IsSpecified || !hasEmbed  && !cachedMsg.Value.Embeds.First().Fields[0].Name.Contains("Giveaway Pool"))
                return;
