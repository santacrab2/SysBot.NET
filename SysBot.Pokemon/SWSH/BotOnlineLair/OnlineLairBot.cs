...
namespace SysBot.Pokemon
{
    ...
    public sealed class onlineLairBot : EncounterBot
    {
        ...
        private readonly OnlineLairBotSettings Settings;
        ...
        public PK8 lairPk = new();
        public PK8 selection = new();
        public uint playeroff;
        public PK8[] party = new PK8[3];
        ...
        public onlineLairBot(PokeBotState cfg, PokeTradeHub<PK8> hub) : base(cfg, hub)
        {
            Settings = Hub.Config.OnlineLair;
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                if (raidCount == 1)
                {
                    ...
                    Log($"Starting a Solo Adventure for {(species == 0 ? "a random Legendary" : (Species)species)}!{winRate}");
                        ...
                        EchoUtil.Echo($"Starting a Max Adventure for {(species == 0 ? "a random Legendary" : (Species)species)}!\nLink Code: {(Hub.Config.OnlineLair.OnlineSettings.LairOnlineCode >= 0 && nogocount <= Hub.Config.OnlineLair.OnlineSettings.LairOnlineNoGoMax ? Hub.Config.OnlineLair.OnlineSettings.LairOnlineCode.ToString().Insert(4, "-") : "no code") }");
                        System.IO.File.WriteAllText($"{System.IO.Directory.GetCurrentDirectory()}//Lairtext.txt", $"Max Adventures: {(Species)species}\nLink Code: {(Hub.Config.OnlineLair.OnlineSettings.LairOnlineCode >= 0 && nogocount <= Hub.Config.OnlineLair.OnlineSettings.LairOnlineNoGoMax ? Hub.Config.OnlineLair.OnlineSettings.LairOnlineCode.ToString().Insert(4, "-") : "no code") }");
                        var timetowait = Hub.Config.OnlineLair.OnlineSettings.LairOnlineMinWait * 1_000;
                        ...
                        var pk = await ReadUntilPresent(RentalMon1, 2_000, 0_200, 344, token).ConfigureAwait(false);
                        while (pk == null)
                        {


                            pk = await ReadUntilPresent(RentalMon1, 2_000, 0_200, 344, token).ConfigureAwait(false);
                            if (pk != null)
                                break;
                            Log("Entered the lobby too fast, correcting... or waiting for online");
                            await Click(A, 500, token).ConfigureAwait(false);
                ...
                while (await LairStatusCheck(OffsetValues.LairLobby, CurrentScreenLairOffset, token).ConfigureAwait(false)) // Delay until in path select screen.
...
#pragma warning disable CS8601 // Possible null reference assignment.
                    ...
                    if (raidCount == 1)
                    {
                        party = new PK8[3] { await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                        PlayerPk = await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false);
                        if (PlayerPk == null)
                            PlayerPk = new();
                        playeroff = LairPartyP1Offset;
                        if (PlayerPk.DisplaySID != Hub.Config.OnlineLair.OnlineSettings.lairBotSID)
                        {
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                            PlayerPk = await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false);
                            if (PlayerPk == null)
                                PlayerPk = new();
                            playeroff = LairPartyP2Offset;
                        }
                        if (PlayerPk.DisplaySID != Hub.Config.OnlineLair.OnlineSettings.lairBotSID)
                        {
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                            PlayerPk = await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false);
                            if (PlayerPk == null)
                                PlayerPk = new();
                            playeroff = LairPartyP3Offset;
                        }
                        if (PlayerPk.DisplaySID != Hub.Config.OnlineLair.OnlineSettings.lairBotSID)
                        {
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                            PlayerPk = await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false);
                            playeroff = LairPartyP4Offset;
                        }
                    ...
                    {
                        PlayerPk = await ReadUntilPresent(playeroff, 0_500, 0_200, 344, token).ConfigureAwait(false);
                        if (playeroff == LairPartyP1Offset)
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                        else if (playeroff == LairPartyP2Offset)
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                        else if (playeroff == LairPartyP3Offset)
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP4Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
                        else
                            party = new PK8[3] { await ReadUntilPresent(LairPartyP1Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP2Offset, 0_500, 0_200, 344, token).ConfigureAwait(false), await ReadUntilPresent(LairPartyP3Offset, 0_500, 0_200, 344, token).ConfigureAwait(false) };
...
#pragma warning restore CS8601 // Possible null reference assignment.
                ...
                Log($"Raid Battle {raidCount}. Encounter {LairEncounterCount}: {SpeciesName.GetSpeciesNameGeneration(lairPk.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(lairPk.Species, lairPk.Form, out _)}.");
                    ...
                    EchoUtil.Echo($"Raid Battle {raidCount}. Encounter {LairEncounterCount}: {SpeciesName.GetSpeciesNameGeneration(lairPk.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(lairPk.Species, lairPk.Form, out _)}.");
                if (PlayerPk == null)
                    PlayerPk = new();
                ...
                Log($"Sending out: {SpeciesName.GetSpeciesNameGeneration(PlayerPk.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(PlayerPk.Species, PlayerPk.Form, out _)}.");
                    ...
                    EchoUtil.Echo($"Sending out: {SpeciesName.GetSpeciesNameGeneration(PlayerPk.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(PlayerPk.Species, PlayerPk.Form, out _)}.");
                ...
                if (Lost) // We've lost the battle, exit back to main loop.
                {
                    Log($"Lost Adventure {AdventureCounts.AdventureCount}.");
                        ...
                        EchoUtil.Echo($"Lost Adventure {AdventureCounts.AdventureCount}.");
                    if (Caught > 0)
        ...
        private async Task RentalRoutine(ushort noteSpecies, CancellationToken token)
        {
            selection = new();
                ...
                EchoUtil.Echo($"Starting Max Adventure for {(Species)noteSpecies}");
            ...
            int monIndex = -1;
            while (await LairStatusCheck(OffsetValues.LairLobby, CurrentScreenLairOffset, token).ConfigureAwait(false))
            {
                ...
                selection = pkList[monIndex == -1 ? speedIndex : monIndex];
                Log($"Selecting {SpeciesName.GetSpeciesNameGeneration(selection.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(selection.Species, selection.Form, out _)}.");
                    ...
                    EchoUtil.Echo($"Trying to Select {SpeciesName.GetSpeciesNameGeneration(selection.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(selection.Species, selection.Form, out _)}.");
                await MoveAndRentalClicks(monIndex == -1 ? speedIndex : monIndex, token).ConfigureAwait(false);
                var temprental1 = await ReadUntilPresent(RentalMon1, 2000, 200, 344, token).ConfigureAwait(false);
                if (temprental1 == null)
                    return;
                var temprental2 = await ReadUntilPresent(RentalMon2, 2000, 200, 344, token).ConfigureAwait(false);
                if (temprental2 == null)
                    return;
                var temprental3 = await ReadUntilPresent(RentalMon3, 2000, 200, 344, token).ConfigureAwait(false);
                if (temprental3 == null)
                    return;
                int[] temprentals = new int[3] { temprental1.Species, temprental2.Species, temprental3.Species };
                if (!temprentals.Contains(selection.Species))
                    return;
            }
        }
        ...
        private async Task<LairBotUtil.PokeMoveInfo> SelectMove(PK8[] party, PK8 lairMon, bool stuck, int turn, bool dmax, bool dmaxEnded, CancellationToken token)
        {
            ...
            Log($"Turn {turn}: Selecting {(dmax ? (DmaxMoves)dmaxMove : (Move)PlayerPk.Moves[bestMove])}.");
                ...
                EchoUtil.Echo($"Turn {turn}: Selecting {(dmax ? (DmaxMoves)dmaxMove : (Move)PlayerPk.Moves[bestMove])}.");
        ...
        private bool CheckIfUpgrade(PK8[] party, PK8 lairPk)
        {
            ...
            if (upgrade && lairPk.Species != LairBoss.Species)
            {
                Log("Lair encounter is better than our current Pokémon. Going to catch it and swap our current Pokémon.");
                    ...
                    EchoUtil.Echo("Lair encounter is better than our current Pokémon. Going to catch it and swap our current Pokémon.");
            }
        ...
        private async Task BattleRoutine(PK8[] party, PK8 lairPk, CancellationToken token)
        {
            ...
            while (true)
            {
                    ...
                    if (await LairStatusCheckMain(OffsetValues.LairBattleMenu, LairMiscScreenCalc, token).ConfigureAwait(false))
                    {
                        ...
                        if (!await LairStatusCheckMain(OffsetValues.LairMovesMenu, LairMiscScreenCalc, token).ConfigureAwait(false) && !fainted)
                        {
                            Log($"Turn {turn}: Cheering on...");
                                ...
                                EchoUtil.Echo($"Turn {turn}: Cheering on...");
                            fainted = true;
                            canDmax = false;
                            dmaxEnd = 0;
                        }
                ...
                var newPlayerPk = await ReadUntilPresent(playeroff, 2_000, 0_200, 344, token).ConfigureAwait(false);
                ...
                if (!Settings.EnableOHKO && !dmax && canDmax)
                {
                    ...
                    Log(PlayerPk.CanGigantamax ? "Gigantamaxing..." : "Dynamaxing...");
                        ...
                        EchoUtil.Echo(PlayerPk.CanGigantamax ? "Gigantamaxing..." : "Dynamaxing...");
                    dmax = true;
                    canDmax = false;
                    dmaxEnd = 3;
                }
        ...
        private async Task CatchRoutine(int raidCount, PK8[] party, PK8 lairPk, CancellationToken token)
        {
            ...
            if (Settings.CatchLairPokémon || upgrade || raidCount == 4) // We want to catch the legendary regardless of settings for catching.
            {
                ...
                Log($"Catching {(raidCount < 4 ? "encounter" : "legendary")}...");
                    ...
                    EchoUtil.Echo($"Catching {(raidCount < 4 ? "encounter" : "legendary")}...");
            ...
            Log($"{(raidCount == 4 || Settings.CatchLairPokémon || upgrade ? "Caught" : "Defeated")} {SpeciesName.GetSpeciesNameGeneration(lairPk.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(lairPk.Species, lairPk.Form, out _)}.");
                ...
                EchoUtil.Echo($"{(raidCount == 4 || Settings.CatchLairPokémon || upgrade ? "Caught" : "Defeated")} {SpeciesName.GetSpeciesNameGeneration(lairPk.Species, 2, 8)}{TradeExtensions<PK8>.FormOutput(lairPk.Species, lairPk.Form, out _)}.");
        }
        ...
        private async Task Results(CancellationToken token)
        {
            ...
            if (index == -1)
            {
                ...
                Log("No results found... Going deeper into the lair...");
                    ...
                    EchoUtil.Echo("No results found... Going deeper into the lair...");
                return;
            }
        ...
        private async Task<bool> LegendReset(CancellationToken token)
        {
            ...
            Log("Reading legendary Pokémon offset...");
                ...
                EchoUtil.Echo("Reading legendary Pokémon offset...");
            ...
            Log($"Reset {ResetCount} {Environment.NewLine}{ShowdownParsing.GetShowdownText(LairBoss)}{Environment.NewLine}");
                ...
                EchoUtil.Echo($"Reset {ResetCount} {Environment.NewLine}{ShowdownParsing.GetShowdownText(LairBoss)}{Environment.NewLine}");
            ...
            Log("Stats match conditions, now let's continue the adventure and check if it's shiny...");
                ...
                EchoUtil.Echo("Stats match conditions, now let's continue the adventure and check if it's shiny...");
        ...
        private async Task SelectCatchingBall(CancellationToken token)
        {
            Log($"Selecting {Settings.LairBall} Ball...");
                ...
                EchoUtil.Echo($"Selecting {Settings.LairBall} Ball...");
            await Click(A, 0_500, token).ConfigureAwait(false);
        ...
        private async Task LairEntry(CancellationToken token)
        {
            ...
            await Task.Delay(2_000).ConfigureAwait(false);
                ...
                if (Hub.Config.OnlineLair.OnlineSettings.LairOnlineCode >= 0 && nogocount <= Hub.Config.OnlineLair.OnlineSettings.LairOnlineNoGoMax)
                {
                    // Set Link code
                    await Click(PLUS, 1_000, token).ConfigureAwait(false);
                    await EnterLinkCode(Hub.Config.OnlineLair.OnlineSettings.LairOnlineCode, Hub.Config, token).ConfigureAwait(false);
                    await Click(PLUS, 2_000, token).ConfigureAwait(false);
                    await Click(A, 2_000, token).ConfigureAwait(false);
                    if (nogocount > Hub.Config.OnlineLair.OnlineSettings.LairOnlineNoGoMax)
                        nogocount = 1;
                }
